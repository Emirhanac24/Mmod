...
module.exports = {
    description: 'Dark Zone V13 Mod| Kick Komutu',
  
    options:[{
      name: "kullanıcı",
      description: "Bir kullanıcı etiketlendiğinde o kullanıcıyı sunucu'dan atar!",
      type: "USER",
      required: true
    }],
 ...
 const m = interaction.options.getMember('kullanıcı')
  const ml = MLOG.findOne({ guildID: interaction.guild.id });
   ...
   if(!m){return interaction.reply({ content: "Belirtilen ID'ye sahip kullanıcı bulunamadı!", ephemeral: false})}
      m.kick(["Sebep Girebilirsiniz!"])
        const embed = new MessageEmbed()
        .setTitle('Dark Zone V13 Mod| Kick Komutu', true)
        .setDescription("Başarıyla kullanıcı atıldı!")
